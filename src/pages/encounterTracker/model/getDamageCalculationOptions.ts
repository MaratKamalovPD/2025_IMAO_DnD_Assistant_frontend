...
export function getDamageCalculationOptions(
  ...
  if (
    creature.damageImmunities.some((r) => checkResistance(r, normalizedDamageType, magicAttack))
  ) {
    return {
      modifier: 'immunity',
      saveEffect: 'full',
  ...
  if (
    creature.damageVulnerabilities.some((r) =>
      checkResistance(r, normalizedDamageType, magicAttack),
    )
  ) {
    return {
  ...
  if (
    creature.damageResistances.some((r) => checkResistance(r, normalizedDamageType, magicAttack))
  ) {
    return {
      modifier: 'resistance',
      saveEffect: 'full',
