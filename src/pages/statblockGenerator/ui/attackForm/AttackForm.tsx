...
export const AttackForm = ({ ref }: { ref?: React.RefObject<CollapsiblePanelRef | null> }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isMultiAttack) {
      ...
      const exists = attacksList.some(
        (a, i) => a.name.toLowerCase() === nameLower && i !== editIndex,
      );
      ...
      const exists = attacksList.some((a) => a.name.toLowerCase() === attack.name.toLowerCase());
  ...
  return (
    <CollapsiblePanel ref={ref} title='Добавить атаку'>
      <form onSubmit={handleSubmit}>
        ...
        {isMultiAttack ? (
          ...
          <>
            ...
            <AttackDamageParams damage={attack.damage} onInputChange={handleDamageInputChange} />
      ...
      {attacksList.length > 0 && (
        <AttackList attacks={attacksList} onEdit={handleEditAttack} onRemove={handleRemoveAttack} />
      )}
    </CollapsiblePanel>
  );
};
