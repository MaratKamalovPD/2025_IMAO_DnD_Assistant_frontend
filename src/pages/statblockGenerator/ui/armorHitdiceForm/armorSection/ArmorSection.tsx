...
const getArmorTypeFromName = (name: string, armorTypes: Record<string, ArmorTypeInfo>): ArmorType => {
  const lowerName = name.toLowerCase();
  const match = Object.entries(armorTypes).find(([, info]) => info.label.toLowerCase() === lowerName);
...
export const ArmorSection: React.FC<{ language: Language }> = ({ language }) => {
  ...
  const hasShield = armors.some(a => typeof a.name === 'string' && a.name.toLowerCase() === t.shield.toLowerCase());
  ...
  armorText !== ''
    ...
    : getArmorTypeFromName(
        armors.find(a =>
          typeof a.name === 'string' && a.name.toLowerCase() !== t.shield.toLowerCase()
        )?.name ?? '',
  ...
  const mergedArmorTypes: Record<string, { label: string; description: string }> = {
    'natural armor': { label: 'Природный доспех', description: '' },
    ...t.armorTypes
  };

  useEffect(() => {
    const label = (t.armorTypes as Record<string, { label: string; description: string }>)[localArmorType]?.label ?? localArmorType;
  ...
  const showNatArmor = getArmorTypeFromName(
    (t.armorTypes as Record<string, { label: string; description: string }>)[localArmorType]?.label ?? localArmorType,
    {
      ...t.armorTypes,
